    ...
    class IncludesMatcher < Matcher
      ...
      def matches?(actual)
        if actual.is_a?(String) || actual.is_a?(Array)
          ...
          content = actual.read
          actual.rewind
          LlmSpec::Runtime::TestResult.new(comment: nil, pass: content.include?(@expected))
        elsif actual.is_a?(PDF::Reader)
          LlmSpec::Runtime::TestResult.new(comment: nil, pass: actual.pages.any? { |page| page.text.include?(@expected) })
        else
